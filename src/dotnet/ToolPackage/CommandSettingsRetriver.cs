...
{
    // TODO wul find some way to dedup with ToolPackageInstance
    internal static class CommandSettingsRetriver
    {
            ...
            bool commandsInNuGetCache)
        {
            ...
            try
            {
                ...
                var library = FindLibraryInLockFile(lockFile, packageId.ToString());
                var dotnetToolSettings = FindItemInTargetLibrary(library, ToolSettingsFileName);
                if (dotnetToolSettings == null)
                {
                    throw new ToolPackageException(
                        string.Format(
                            "Missing setting file", // TODO wul fix loc
                            packageId.ToString()));
                }
